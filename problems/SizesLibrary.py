...
A library of building blocks that uses device sizes (W's, L's, etc), as
...
#NOTE: the MOS model names are defined in util.constants
...
def Point18SizesLibrary():
    ...
    @notes          
    """ 
    ss = SizesLibraryStrategy(DevicesSetup('UMC180'))
...
class SizesLibraryStrategy:
    ...
    def __init__(self, devices_setup):
        ...
        #
        self.feature_size = devices_setup.featureSize()
        self.vdd = devices_setup.vdd()
...
class SizesLibrary(Library):
    ...
    def sphere2dPart(self):
        """
        Description: special part for 2d-sphere function
        Ports: 1,2
        Variables: x0, x1, each in range [-10, +10]
        """
        name = whoami()
        if self._parts.has_key(name):  return self._parts[name]
        point_meta = PointMeta([ContinuousVarMeta(False, -10.0, +10.0, 'x0'),
                                ContinuousVarMeta(False, -10.0, +10.0, 'x1')] )
        part = AtomicPart('M', ['1','2'], point_meta, name)
