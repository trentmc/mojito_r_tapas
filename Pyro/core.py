#############################################################################
#  
#	$Id: core.py,v 2.104.2.13 2008/05/24 13:31:47 irmen Exp $
#	Pyro Core Library
#
...
import Pyro.constants, Pyro.util, Pyro.protocol
...
_remoteImportLock=Pyro.util.getLockObject()
...
class DynamicProxy:
	...
	def _transferThread(self, newOwnerThread=None):
		...
		elif Pyro.util.supports_multithreading():
			self.ownerThreadId = id(threading.currentThread())
		else:
			self.ownerThreadId = None
	def _release(self):
	...
	def _invokePYRO(self, name, vargs, kargs):
		if Pyro.util.supports_multithreading() and not Pyro.config.PYRO_PROXYSHARING_ENABLED:
...
class Daemon(Pyro.protocol.TCPServer, ObjBase):
	def __init__(self,prtcol='PYRO',host=None,port=0,norange=0,publishhost=None):
		...
		if norange:
			...
			# attempt to use the provide port value
			self.port = port
			portrange = 1
		else:
