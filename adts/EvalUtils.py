...
def getSpiceData(filename, number_width, start_line, num_vars):
    ...
    @notes
      ...
      -there is always one extra var present in the file, but we only
       return it if it's the time variable (it will be 0th entry).  
    ...
    X = numpy.zeros((num_vars+1, num_points), dtype=float)
